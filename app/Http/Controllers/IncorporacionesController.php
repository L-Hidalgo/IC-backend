...
class IncorporacionesController extends Controller
{
    public function crearActualizarIncorporacion(Request $request)
    {
        $validatedData = $request->validate([
            ...
            'obsEvaluacionDetalleIncorporacion' => 'nullable|string',
        ...
        if (isset($validatedData['puestoNuevoId']) && isset($validatedData['personaId'])) {
            ...
            if ($puesto) {
                ...
                if (isset($validatedData['obsEvaluacionDetalleIncorporacion'])) {
                    $incorporacion->obs_evaluacion_detalle_incorporacion = $validatedData['obsEvaluacionDetalleIncorporacion'];
                }
    ...
    public function generarR1469($incorporacionId)
    {
        ...
        $nombreGerencia = mb_strtoupper($incorporacion->puesto_nuevo->departamento->gerencia->nombre_gerencia, "UTF-8");
        $templateProcessor->setValue('puestoNuevo.gerencia', $nombreGerencia);

        $departamento = mb_strtoupper($incorporacion->puesto_nuevo->departamento->nombre_departamento, "UTF-8");
        $templateProcessor->setValue('puestoNuevo.departamento', $departamento);
